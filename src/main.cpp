...
int main(/*int argc, char **argv*/) {
    ...
    char graphName[80] = "flat1000_50_0";
    int nbColor = 55;
...
// void printhelp(char *prog) {
//     printf(
//         "Usage:  %s [options] <dimacs-file> <number-color>\n"
//         "\n"
//         " <dimacs-file>   is the ASCII DIMACS-format file from which to read the
//         graph.\n" " <number-colors> is the number of colors used.\n Options are the
//         following "
//         "(I:Integer; F:Float; S:String):\n"
//         "\n"
//         " -h    --help           This help text.\n"
//         " -t    --tabucol        Run TabouCol (with 10 000 000 iterations by
//         default)\n" "                         instead of HEAD (default: HEAD).\n" " -i
//         I  --iter I         Number of iterations for the local search\n" " (default
//         =30000).\n" " -g I  --gen I          Number of generations of memetic
//         algorithm\n" "                         (-1: until finding a legal coloring;
//         default =-1).\n" " -r I  --run I          Number of runs (-1: run until finding
//         a\n" "                         legal coloring; default =1)\n" " -d I  --seed I
//         Seed to run again a specific run\n" "                         (0: random seed,
//         default)\n" " -R I  --RandCross I    Number of random color classes transmited
//         before GPX\n" "                         crossover (default=0: standard GPX
//         crossover).\n" " -w I  --swap I         Number of runs before swapping \n" "
//         (-1 : no swapping; default =-1).\n" " -p F  --swapingRate F  Proximity that
//         generates a swap (defaut =99).\n" " -a F  --acceptRate F   Acceptance rate
//         [0.0, 1.0] of a chirld worst than \n" "                         parents
//         (default=1.0).\n" " -U F  --weightParent F Imbalanced crossover. Weight [0,
//         100] of first parent\n" "                         in crossover (defaut =-1 : no
//         weight = standard GPX).\n" " -s I  --maxseconds I   Maximal duration of one run
//         (defalut =-1 : no limit).\n" "                         Another stopping
//         criterion is when the 4 colorings\n" "                         (2 solutions + 2
//         elites) are too similar.\n" " -o S  --output S       Save solution(s) found in
...
//             {0, 0, 0, 0}};
//         c = getopt_long(
//             argc, argv, "i:g:r:d:R:w:p:a:U:s:f:o:qth", long_options, &option_index);
//         if (c == -1)
//             break;
//         switch (c) {
//         case 0:
...
//             if ((*optarg == 0) || (*ptr != 0) || (*swapingRate <= 0) ||
//                 (*swapingRate > 100)) {
...
//             if ((*optarg == 0) || (*ptr != 0) || (*tauxAcceptWorst < 0) ||
//                 (*tauxAcceptWorst > 1)) {
...
//             if ((*optarg == 0) || (*ptr != 0) || (*weightParent < -1) ||
//                 (*weightParent > 100)) {
...
//             exit(1);
//         default:
//             fprintf(stderr, "Internal bug: line %d should never be reached!",
//             __LINE__); exit(-1);
//         }
//     }
